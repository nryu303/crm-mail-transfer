...
MAX_FETCH    = 50  # per-account safety cap per run. Raised back to 50 on 2026-05-28
                   # after pre-flagging Postmaster bounces as \Seen at IMAP level
                   # (see drain_bounces() below) kept the backlog small. 50 × 43
                   # accounts × 30 ticks/hour = 64,500/hr theoretical; real-world
                   # is much lower because most ticks find 0 unseen for most accts.
                   # If memory pressure returns, lower this OR tighten drain_bounces().
...
def drain_bounces(conn, user):
    """Bulk-flag Postmaster/Mailer-Daemon delivery-failure mails as \\Seen at IMAP
    level WITHOUT fetching their bodies. CRM rejects them anyway as
    REASON_BOUNCE; pulling 600+ bounces just to throw them away clogs the
    fetcher's per-tick budget and delays real user replies behind hours of
    backlog (incident 2026-05-28: hh.88798087→ycxaieika took 2h to surface
    because 670 Postmaster mails were ahead in the UNSEEN queue).

    Uses IMAP SEARCH with FROM-keyword filter then UID STORE +FLAGS \\Seen,
    a fast metadata-only operation. Returns the number of UIDs flagged.
    Failures are logged but non-fatal — the normal fetch path picks them up
    on the next tick.
    """
    flagged = 0
    for keyword in ("postmaster", "mailer-daemon"):
        try:
            ...
            if typ == "OK":
                flagged += len(uids)
                # Also persist to our state file so future ticks don't re-evaluate.
                for u in uids:
...
def run_account(user, password):
    ...
    try:
        ...
        conn.select(MAILBOX)
        # Pre-pass: clear Postmaster/Mailer-Daemon bounces from the UNSEEN queue at IMAP
        # level so real user replies don't queue behind a multi-hundred bounce backlog.
